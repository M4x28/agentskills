...
description: >
  ...
  Use when the user asks to "make a presentation", "create slides", "build a deck",
  or mentions .pptx. Trigger with "create a presentation about X", "make slides
  for Y", or /pptx.
parameters:
  required:
    - name: topic
...
3. Tailor language and depth to `audience` (default: general audience).
4. Structure: title slide → agenda → content slides → summary or call to action.
5. Each slide: one clear heading, 3–5 concise bullet points, and speaker notes.

Deliver the result as a `.pptx` file using the available toolchain.
