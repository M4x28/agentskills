...
| `parameters` | No | Structured parameter definitions for client-side auto-fill. See [Skill Parameters](/skill-creation/parameters). |
...
The optional `parameters` field enables **client-side guided input** when users invoke
a skill via `/skill-name`. Supporting clients render a prompt template with required
and optional parameter fields before the model ever sees the message.

See the [Skill Parameters](/skill-creation/parameters) guide for full schema
documentation, validation rules, and examples.
...
<Note>
Clients that do not implement this extension safely ignore the `parameters:` key and invoke the skill normally.
</Note>
